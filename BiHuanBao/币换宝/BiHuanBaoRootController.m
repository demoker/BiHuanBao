...
#import "KxMenu.h"
@interface BiHuanBaoRootController ()

@end

@implementation BiHuanBaoRootController
@synthesize navBar;
...
- (void)download_productTypeList
{
    ...
    {
        url = [NSString stringWithFormat:@"%@%@&row=10&page=%d&auto_code=%@",HTTP,BiHuanBaoListUrl,more,self.default_code_id];
    }
    else
    {
        url = [NSString stringWithFormat:@"%@%@&row=10&page=%d",HTTP,BiHuanBaoListUrl,more];
    }
    [[ELHttpRequestOperation sharedClient] GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSArray * arr = (NSArray *)responseObject;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([productArray count]!=0)
    {
        ...
        NSRange range = [[NSString stringWithFormat:@"总成交:%@件",item.content_sale] rangeOfString:item.content_sale];
        NSMutableAttributedString *attriString = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"总成交:%@件",item.content_sale]];
        //把this的字体颜色变为红色
        [attriString addAttribute:NSForegroundColorAttributeName
                            value:UIColorFromRGB(0xff602a)
                            range:NSMakeRange(range.location, range.length)];
        cell.numbers.attributedText = attriString;
...
- (void)ad_request
{
    NSString * url = [NSString stringWithFormat:@"%@%@&optionid=%@",HTTP,AdvantageShowUrl,@"999"];
    [[ELHttpRequestOperation sharedClient] GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSArray * arr = (NSArray *)responseObject;
        if(arr == nil)
        {
