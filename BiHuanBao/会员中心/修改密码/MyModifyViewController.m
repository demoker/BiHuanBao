        ...
        if(self.type == 10)//修改密码
            {
                ...
                if(is_Remember_Bool)
                {
                    if([[UserLoginInfoManager loginmanager] isCorper])
                    {
                        [[NSUserDefaults standardUserDefaults] setValue:_TextFieldTwo.text forKey:@"corp_pwd"];
                    }
                    else
                    {
...
- (void)home:(id)sender
{
    ...
    [_TextFieldTwo resignFirstResponder];
    [_TextFieldThree resignFirstResponder];
    NSString *  url = [NSString stringWithFormat:@"%@%@",HTTP,ModifyPassWordUrl];
    NSMutableDictionary * dic = [[NSMutableDictionary alloc] init];
