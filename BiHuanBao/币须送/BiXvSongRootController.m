...
#import "BixvSongRootViewCell.h"
#import "BixvSongFinalViewController.h"
#import "KxMenu.h"
@interface BiXvSongRootController ()

@end

@implementation BiXvSongRootController
@synthesize navBar;
...
- (void)download_productTypeList
{
    ...
    {
        url = [NSString stringWithFormat:@"%@%@&row=10&page=%d&auto_code=%@",HTTP,BiXvSongListUrl,more,self.default_code_id];
    }
    else
    {
        url = [NSString stringWithFormat:@"%@%@&row=10&page=%d",HTTP,BiXvSongListUrl,more];
    }
    [[ELHttpRequestOperation sharedClient] GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSArray * arr = (NSArray *)responseObject;
        for(NSDictionary * dic in arr)
        {
            ...
            item.sub_title = OBJC([dic objectForKey:@"content_desc"]);
...
{
    BixvSongRootViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"BixvSongRootViewCell"];
    if(cell == nil)
    {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"BixvSongRootViewCell" owner:self options:nil] lastObject];
    ...
    if([productArray count]!=0)
    {
        ...
        cell.content_name.text = item.content_name;[cell.content_name sizeToFit];
...
{
    BixvSongFinalViewController * final = [[BixvSongFinalViewController alloc]initWithNibName:@"BixvSongFinalViewController" bundle:nil];
    ProductListItem * item = [productArray objectAtIndex:indexPath.row];
...
- (void)ad_request
{
    NSString * url = [NSString stringWithFormat:@"%@%@&optionid=%@",HTTP,AdvantageShowUrl,@"1000"];
    [[ELHttpRequestOperation sharedClient] GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSArray * arr = (NSArray *)responseObject;
        if(arr == nil)
        {
...
- (void)loginfinished
{
//    MemberCenterController * member = [[MemberCenterController alloc]init];
//    member.titleName = @"会员中心";
