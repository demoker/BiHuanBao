...
{
    bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.Sframe.size.width, self.Sframe.size.height)];
    abnormalButton.center = CGPointMake(bgView.frame.size.width/2,bgView.frame.size.height/2);
    refreshSpinnerView.center = CGPointMake(self.Sframe.size.width/2, self.Sframe.size.height/2);
...
- (void)abnormalButtonShow:(UIImage *)image text:(NSString *)text
{
    ...
    abnormalButton.titleLabel.font = [UIFont boldSystemFontOfSize:14];
