...
- (void)initTheInterface
{
    ...
    self.titleName.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"nav_bg"]];
...
{
    [self animatedOut];
    [self.delegate performSelector:@selector(coverDidhidden) withObject:nil];
}
...
- (void)touchForDismissSelf:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(reloadRequest)]) {
        [self.delegate performSelector:@selector(reloadRequest)];
    }
}
