...
- (void)download
{
    ...
    [[ELHttpRequestOperation sharedClient] GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.contact = OBJC([dic objectForKey:@"content_contact"]);
        self.content_titleImg = OBJC([dic objectForKey:@"content_face"]);
        self.contact_link = OBJC([dic objectForKey:@"content_linkname"]);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell == nil)
    {
        ...
        cell.contact_link.delegate = self;
...
//        cell.email.background = [UIImage imageNamed:@"login_input.png"];
        cell.username.background = [UIImage imageNamed:@"login_input.png"];
//        cell.mobile.background = [UIImage imageNamed:@"login_input.png"];
        ...
        cell.contact_link.background = [UIImage imageNamed:@"login_input.png"];
...
//        cell.email.background = nil;
        ...
        cell.contact_link.userInteractionEnabled = NO;
    }
    ...
    cell.contact_link.text = self.contact_link;
...
{
    if(textField.tag == 100)//注册邮箱
    {
        self.email = textField.text;
    }
    else if (textField.tag == 101)//用户名
    {
        self.username = textField.text;
    }
    else if (textField.tag == 102)//绑定手机
    {
        self.mobile = textField.text;
    }
    else if(textField.tag == 103)//默认收货人
    {
        self.contact = textField.text;
    }
    else if (textField.tag == 104)//默认联系方式
    {
        self.contact_link = textField.text;
    }
    
    //test.b-bao.com/app/index.php?com=com_appService&method=save&app_com=com_passport&task=app_editInfo&ID=demoker@qq.com&PWD=202CB962AC59075B964B07152D234B70&frm[content_name]=demoker@qq.com&frm[content_contact]=erer&frm[content_linkname]=15677889900&frm[content_address]=ciyunsi
}
...
- (void)commitAction:(UIButton *)sender
{
    ...
    if(is_edit)
    {
        ...
        NSString * url = [NSString stringWithFormat:@"%@%@&ID=%@&PWD=%@&frm[content_name]=%@&frm[content_contact]=%@&frm[content_linkname]=%@&frm[content_address]=%@",HTTP,UserInfoEditUrl,[[UserLoginInfoManager loginmanager] user],[[UserLoginInfoManager loginmanager] pwd],self.username,[self.contact stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],self.contact_link,[self.address stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        NSLog(@"%@",url);
...
- (void)imageCropper:(VPImageCropperViewController *)cropperViewController didFinished:(UIImage *)editedImage {
    ...
    [cropperViewController dismissViewControllerAnimated:YES completion:^{
        // TO DO
        UserInfoTableViewCell * cell = (UserInfoTableViewCell *)[_mtableview cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
